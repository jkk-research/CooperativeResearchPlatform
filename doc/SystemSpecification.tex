...
\begin{itemize}
    \item Use camel case everywhere if not stated otherwise
    \item Classes and structs should start with upper case letter
    \item Methods should start with lower case
    \item Constant variables should be all upper case letters. Instead of camel case the words should be separated with '\_'
    \item In classes, private and protected attributes should start with the 'm\_' prefix
    \item In methods, the parameters should start with the '???' prefix
    \item Pointers should comply with the previous rules but should have a '\_' suffix (or p\_ prefix ???)
    \item Namespaces should be ??? (fully lower/upper case, like classes, ?)
    \item Maximum namespace depth should be ???
...
\begin{itemize}
    ...
    \item have header guards (\#ifndef, \#define, \#endif)
