...
bool crp_msgs__msg__ego__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  ...
  {  // wheel_angle
    PyObject * field = PyObject_GetAttrString(_pymsg, "wheel_angle");
    if (!field) {
      return false;
    }
    assert(PyFloat_Check(field));
    ros_message->wheel_angle = (float)PyFloat_AS_DOUBLE(field);
    Py_DECREF(field);
  }
...
PyObject * crp_msgs__msg__ego__convert_to_py(void * raw_ros_message)
{
  ...
  {  // wheel_angle
    PyObject * field = NULL;
    field = PyFloat_FromDouble(ros_message->wheel_angle);
    {
      int rc = PyObject_SetAttrString(_pymessage, "wheel_angle", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
